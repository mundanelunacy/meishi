...
import { useExtractBusinessCardMutation } from "../card-extraction/openAiApi";
...
export function CaptureWorkspace() {
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      pushToast(`${files.length} image${files.length === 1 ? "" : "s"} added to the active capture session.`);
      setCameraError(null);
    } catch (error) {
      setCameraError(error instanceof Error ? error.message : "Unable to process images.");
    }
  }
  ...
  async function handleExtract() {
    ...
    setCameraError(String(result.error.data ?? "Extraction failed."));
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Use a rear camera on mobile or upload existing photos. Multiple captures stay grouped in one local session.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <label className="flex cursor-pointer flex-col items-center justify-center gap-3 rounded-[28px] border border-dashed border-border bg-background/60 px-6 py-12 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                On supported mobile browsers this opens the rear camera directly.
              </p>
            </div>
            <input
              className="hidden"
          ...
          {cameraError ? <Alert className="border-destructive/30 text-destructive">{cameraError}</Alert> : null}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Captured images are kept in IndexedDB so you can recover the current session after navigation or refresh.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {images.length ? (
            <div className="grid gap-4 sm:grid-cols-2">
              {images.map((image) => (
                <div key={image.id} className="overflow-hidden rounded-[24px] border border-border/70 bg-background/70">
                  ...
                  <div className="space-y-1 p-4 text-xs text-muted-foreground">
                    ...
                    <p>
                      {image.width}×{image.height} • {new Date(image.capturedAt).toLocaleString()}
                    </p>
                  </div>
                </div>
              ))}
            ...
            <Alert>No images captured yet. Start with the camera or photo library above.</Alert>
          )}
        </CardContent>
      </Card>
    </div>
